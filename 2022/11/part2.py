...
def combined_divisors(monkeys):
  return reduce(lambda a,b: a*b[1].test_divisor, monkeys.items(), 1)
...
def calculate(file_name):
  ...
  combined_divisor = combined_divisors(monkeys)

  for r in range(1,10001):
    print(f"\n== Round {r} ==")
    for n, monkey in monkeys.items():
      # print(f"Monkey {n}:")
      monkey.round(monkeys, combined_divisor)
    print_monkeys(monkeys)
